...
def calcfps( self ):
	...
	try:
		...
		fps = self.drawnowtime - self.drawlasttime
		self.drawfpslist.append( 1.0 / fps )
		if len( self.drawfpslist ) > 64:
			del self.drawfpslist[ 0 ]

		fps = self.drawnowtime - self.updatelasttime
		self.totalfpslist.append( 1.0 / fps )
		if len( self.totalfpslist ) > 64:
			del self.totalfpslist[ 0 ]
